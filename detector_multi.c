...
{
    char output_path[]="/home/prizmatics91/testing_darknet/darknet/output_predictions/";
    char ext[]=".txt";
    char new_ext[]="_new";
    char extra_path[] = "inputs/";
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "cfg/obj.names");
    char **names = get_labels(name_list);
    ...
    while(1){
        DIR *d;
        struct dirent *dir;
        d = opendir("/home/prizmatics91/testing_darknet/darknet/inputs/");
        char buff[256];
        char* input = buff;
        char file_name[256] = "/home/prizmatics91/testing_darknet/darknet/";
        //int status;
        if (d)
        {
            //printf("inside d");
            while ((dir = readdir(d)) != NULL)
            {
                ...
                if(strlen(dir->d_name)>3){
                    ...
                    strncpy(input, extra_path, 256);
                    strcat(input, dir->d_name);
                    printf("%s\n", input);
                    strncpy(input2, input, 256);
                    if(!input) return;
                    strtok(input, "\n");
                    int i = 0;
                    ...
                    //printf("%s\n", array2[0]);
                    char output_file[sizeof(output_path)+sizeof(array2[0])+sizeof(new_ext)];
	                char output_text_file[sizeof(output_path)+sizeof(array2[0])+sizeof(ext)];
                    strcpy(output_file, output_path);
	                strcpy(output_text_file, output_path);
                    strcat(output_file, array2[0]);
                    strcat(output_file, new_ext);
	                strcat(output_text_file, array2[0]);
	                strcat(output_text_file, ext);
                    //printf("%s\n", output_file);
                    //puts(output_text_file); 
                    FILE *ptr;   
                    ptr = fopen(output_text_file, "w");
                    sleep(2);
                    ...
                    for(k = 0; k < nboxes; ++k){
                        ...
                        if(class >= 0){
	                        ...
	                        fprintf(ptr, "Bounding Box: Left=%d, Top=%d, Right=%d, Bottom=%d\n\n", left, top, right, bot);
